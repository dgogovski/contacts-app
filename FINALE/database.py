...
class database:
    ...
    def get_user_by_id(user_id):
        with database() as db:
            ...
            if row is None or False:
                app.logger.info('No users to get by this id')
                return None
            return User(*row)
    ...
    def find_by_number(number):
        with database() as db:
            row = db.execute('SELECT * FROM users WHERE number = ?', (number,)).fetchone()
            if row is None:
                return None
            return User(*row)
    ...
    def get_contacts_by_user_id(user):
        if user is not None:
            ...
            with database() as db:
                ...
                # Check if this user has any contacts
                if rows is not False:
                    # Cycle through contacts and update activity status for each one
                    for row in rows:
                        contact = Contact(*row)
                    ...
                    return None
        # User does not exist
        app.logger.error('User with this id does not exist.')
        return False
